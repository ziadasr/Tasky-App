...
interface ManagerOrganizationViewProps {}

export const ManagerOrganizationView: React.FC<
  ManagerOrganizationViewProps
> = () => {
  const [searchQuery, setSearchQuery] = useState<string>("");
  const [sortBy, setSortBy] = useState<
    "name" | "salary" | "createdAt" | "lastLogin"
  >("name");
  ...
  useEffect(() => {
    const fetchEmployees = async () => {
      ...
      try {
        ...
        const data = await apiService.getAllEmployees(
          offset,
          itemsPerPage,
          undefined, // No role filter for managers
          sortBy,
          sortDir
        );
    ...
    fetchEmployees();
  }, [sortBy, sortDir, currentPage, itemsPerPage]);
  ...
  const stats = useMemo(() => {
    ...
    return { totalEmployees, employeeCount, totalSalary, avgSalary };
  }, [employees]);
